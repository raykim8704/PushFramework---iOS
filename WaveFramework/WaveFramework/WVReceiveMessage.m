...
@implementation WVReceiveMessage


-(void)recieveMessage:(NSString*)docID cmpNm:(NSString*)cmpNm sendAt:(NSString*)sendAt agentId:(NSString *)agentId{
    ...
    [self tableRowCheck];
    NSLog(@"Save DOC_ID");
    NSString *query = [NSString stringWithFormat:@"insert into '%@' values(NULL,'%@')",TABLE_NAME_FOR_RECEIVE_DOCID,docID];
    ...
    [WVHTTPCommunicator SendJsonToServer:body urlSuffix:API_SUFFIX_RECIEVE];
    
    }
    else{
        NSLog(@"receive doc_id already exist");
    }
    
}
-(BOOL)isDocIdExist:(NSString*)doc_id{
    WVDatabaseManager *dbManager = [[WVDatabaseManager alloc]init];
    NSString *query = [NSString stringWithFormat: @"SELECT * FROM %@ WHERE doc_id = '%@' ",TABLE_NAME_FOR_RECEIVE_DOCID,doc_id];
...
-(void)tableRowCheck{
     WVDatabaseManager *dbManager = [[WVDatabaseManager alloc]init];
    NSString *query = [NSString stringWithFormat: @"SELECT COUNT(*) FROM '%@' ",TABLE_NAME_FOR_RECEIVE_DOCID];
    NSArray *result= [[NSArray alloc]initWithArray:[dbManager loadDataFromDB:query]];
    if([result count])
    {
    ...
    {
        NSString *qry = [NSString stringWithFormat: @"DELETE FROM '%@' ",TABLE_NAME_FOR_RECEIVE_DOCID];
